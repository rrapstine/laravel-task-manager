<li wire:sortable.item="{{ $task->id }}" wire:key="task-{{ $task->id }}" class="flex py-4 mb-4 last:mb-0 space-between items-center">
  <p wire:sortable.handle class="w-full {{ $task->isComplete ? 'line-through' : '' }} hover:cursor-move">{{ $task->task }}</p>
